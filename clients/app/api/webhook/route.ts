...
async function createOrderInSanity(session: Stripe.Checkout.Session, invoice: Stripe.Invoice | null) {
  const { id, amount_total, currency, metadata, payment_intent, total_details, customer } = session;

  const { orderNumber, customerName, customerEmail, customerId, address } =
    metadata as unknown as Metadata & { address: string };

  const parsedAddress = address ? safeJsonParse(address) : null;

  const lineItemsWithProduct = await stripe.checkout.sessions.listLineItems(id, {
    expand: ["data.price.product"],
  });

  const sanityProducts: Array<{ _key: string; product: { _type: string; _ref: string }; quantity: number }> = [];
  ...
  const orderData = {
    ...
    customerName,
    stripeCustomerId: customer || customerEmail,
  ...
  console.log("Creating order in Sanity:", JSON.stringify(orderData, null, 2));
